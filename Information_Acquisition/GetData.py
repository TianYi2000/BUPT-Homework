...
def getUrls(url):
    ...
    if isinstance(div_bf, bs4.element.Tag):
        ...
        for div in div_a:
            ...
            if('五老' in title):
                print(title + ';' + 'http://www.zgggw.gov.cn/shishixinwen' + link)
                urltxt.write(title + ';' + 'http://www.zgggw.gov.cn/shishixinwen' + link + '\n')
...
for index in range(1,338):
    sleep(2)
    url = "http://www.zgggw.gov.cn/shishixinwen/index_" + str(index) + '.html'
    print(url)
    getUrls(url)
